...
class ConnectFourEnv(gym.Env):
    ...
    def reset(self, player1: Player, player2: Player, board: Optional[np.ndarray] = None): #Egen Reset metode.
        player1.reset()
        player2.reset()
        self.reset(board)

    def step(self, player1: Player, player2: Player) -> ResultType: #Egen step metode. Ikke helt fungerende enda, men kan brukes til å stepp igjennom brettet.
        cp = lambda: self.__current_player
        ...
        act = player1.get_next_action(self.__board * cp())
        act_hist = deque([act], maxlen=2)
        step_result = self._step(act)
        state_hist.append(self.__board.copy())
        player = change_player()
